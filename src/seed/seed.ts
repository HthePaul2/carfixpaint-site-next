...
async function seed() {
  ...
  for (const [index, service] of SERVICES.entries()) {
    const doc = await upsertBySlug(payload, 'services', service.id, {
      name: service.name,
      pageSlug: service.pageSlug ?? service.id,
      icon: service.icon,
      ...
      intro: service.intro,
      whenNeededTitle: service.whenNeededTitle,
      whenNeededItems: service.whenNeededItems?.map((item) => ({ item })),
      ...
      ctaSecondaryLabel: service.ctaSecondaryLabel,
